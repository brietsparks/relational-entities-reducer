import { validateSchema, InvalidSchema, ModelSchema, ReciprocalKeyValidator } from './schema';
import { nonStrings, nonObjects, modelSchema as mockModelSchema } from '../mocks';

describe('model/schema', () => {
  describe('validateSchema', () => {
    test('with valid schema', () => {
      validateSchema(mockModelSchema);
    });
    ...
    describe('throws if multiple foreign keys that point to the same entity do not have a corresponding reciprocalKey', () => {
  ...
  describe('ReciprocalKeyValidator', () => {
    it('does not throw if only one foreign key points to a related entity', () => {
      const validator = new ReciprocalKeyValidator('post');
      validator.addForeignKey('user', 'authorId');
      validator.validate({});
    });
    ...
    it('throws if reciprocal key is undefined', () => {
      ...
      validator.addForeignKey('user', 'editorIds');
      validator.addForeignKey('user', 'authorId');
    ...
    it('throws if reciprocal key is the wrong value', () => {
      ...
      validator.addForeignKey('user', 'authorId', 'authoredPostIds');
      validator.addForeignKey('user', 'editorIds', 'incorrectKey');
    ...
    test('with valid schema', () => {
      ...
      validator.addForeignKey('user', 'authorId', 'authoredPostIds');
      validator.addForeignKey('user', 'editorIds', 'editorPostIds');
